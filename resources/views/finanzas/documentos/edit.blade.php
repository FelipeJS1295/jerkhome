...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 text-light">Editar Documento</h1>
    </div>
        ...
        <div class="card-body">
            <form action="{{ route('documentos.update', $documento->id) }}" method="POST">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control bg-secondary text-light" id="numero_documento" name="numero_documento" value="{{ $documento->numero_documento }}" required>
                </div>
                <div class="form-group mb-3">
                    ...
                    <select class="form-control bg-secondary text-light" id="proveedor_id" name="proveedor_id" required>
                        @foreach($proveedores as $proveedor)
                            <option value="{{ $proveedor->id }}" {{ $documento->proveedor_id == $proveedor->id ? 'selected' : '' }}>{{ $proveedor->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group mb-3">
                    ...
                    <input type="date" class="form-control bg-secondary text-light" id="fecha_documento" name="fecha_documento" value="{{ $documento->fecha_documento }}" required>
                </div>
                <div class="form-group mb-3">
                    ...
                    <input type="date" class="form-control bg-secondary text-light" id="vencimiento" name="vencimiento" value="{{ $documento->vencimiento }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="monto_neto" class="form-label" style="background-color: #343a40; padding: 5px 10px; border-radius: 5px;">Monto Neto</label>
                    <input type="number" class="form-control bg-secondary text-light" id="monto_neto" name="monto_neto" value="{{ $documento->monto_neto }}" required>
                </div>
